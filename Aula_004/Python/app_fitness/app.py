...
magic_if = genai.GenerativeModel(
...
def ia_decision(user):
    ...
    """
    user_decision = magic_if.start_chat(enable_automatic_function_calling=True)
    response = user_decision.send_message(
        f"Dados do cliente: {user}; Regras do negócio: {business_rules}"
    ).text
